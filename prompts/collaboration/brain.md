...
使用当前 frame、冻结经验、既有研究笔记和你主动选择读取的本 Run 公开记录。执行器选项只是建议，不限制研究答案；有根据时可以同意，必要时可以否定前提或保留未知。区分实测、执行器报告和你的推测。未读文件与未知指标不能作为已知事实；不索要或重建内部思维链。若本次提供 research_trace，只有你认为有助于判断时才使用，没有必读要求。compute_jobs 是截至帧边界的受控 Job 账本，unknown 继续保留；exit_code=0 不能单独证明成功。
