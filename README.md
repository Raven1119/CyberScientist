...
仍保留的能力边界：原生内部模型费用/调用量尚不能完整计量；新版受控入口已实施 max_jobs/并发/资源门禁和重启查询对账，本次真实 Job 的结果取回仍未通过。主动绕过入口及同一用户可读的凭据文件不属于 OS 隔离覆盖面。SSE 关闭时限只约束连接排空，不保证所有 HTTP 工作线程立即结束。此前完整 Job→Attempt→评分 Run 使用的是旧运行时；本次新运行时完成模型与 Job 创建往返，但没有产物或 Attempt 闭环。
